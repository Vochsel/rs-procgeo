...
fn full_chain_generate_filter_composite_save() {
    ...
    let blurred = noise.apply(&procgeo_cops::filter::BlurCop, &procgeo_cops::filter::BlurParams {
        blur_type: procgeo_cops::filter::BlurType::Gaussian,
        radius_x: 2.0,
        radius_y: 2.0,
    }).unwrap();
    let swirled = blurred.apply(&procgeo_cops::filter::SwirlCop, &procgeo_cops::filter::SwirlParams {
        angle: 45.0,
        radius: 0.5,
        ..Default::default()
    }).unwrap();
...
fn registry_round_trip() {
    ...
    let img = registry.execute("constant", &ctx, &[], r#"{"color":[0.5,0.5,0.5,1.0],"width":8,"height":8}"#).unwrap();
    let flipped = registry.execute("flip", &ctx, &[&img], r#"{"horizontal":true}"#).unwrap();
...
fn all_cops_listed() {
    ...
    for expected in ["constant", "checkerboard", "noise", "ramp", "load_image", "flip", "mirror", "channel_swap", "blur", "swirl", "rotate", "resize", "composite", "custom_shader"] {
